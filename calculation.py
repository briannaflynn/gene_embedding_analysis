...
for gene in domain:
	...
	if non_zero_count < 1500:
		too_many_zero_genes.add(gene)

print("genes with less than 1500 observations:", len(too_many_zero_genes), file=sys.stderr)
domain = set(domain) - too_many_zero_genes
print(f"gene with at least 1500 observations: {len(domain)}", file=sys.stderr)

domain = domain & set(adj_ensID_BPterms)
print(f"gene with at least 1500 observations and proper GOBP terms: {len(domain)}", file=sys.stderr)
...
print("Spearman corr calculation finished", file=sys.stderr)

gene_sorted_corr = dict(sorted(gene_gene2_corr.items(), key=lambda item: item[1], reverse=True))
del gene_gene2_corr

print("Sorting finished", file=sys.stderr)
...
print(f"number of gene pairs considered: {len(total)}", file=sys.stderr)
...
if len(positive_set) > 0:
	...
	AUPRC_0_1p = cal_AUPRC_percent(total, positive_set, 0.1)
else:
	AUROC = np.nan
	AUPRC = np.nan
	AUPRC_1p = np.nan
...
print(f"AUPRC_0.1%: {AUPRC_0_1p:.3f}", file=sys.stderr)
